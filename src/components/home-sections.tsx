    ...
    <section aria-labelledby="hero-title" className="relative overflow-hidden">
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-6 pb-24 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:pt-24">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--muted)]">{copy.heroText}</p>
          <div className="mt-9 flex flex-wrap gap-3">
            <Button href="#trial">
              {copy.heroPrimary} <ArrowRight size={17} aria-hidden="true" />
            </Button>
            <Button href="#programs" variant="secondary">{copy.heroSecondary}</Button>
          </div>
          <ul className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm text-[var(--muted)]">
            {["Qualified teachers", "Flexible schedules", "One-to-one learning"].map((item) => (
              <li key={item} className="flex items-center gap-2">
                <CheckCircle2 size={17} className="text-[var(--green)]" aria-hidden="true" />{item}
              </li>
            ))}
          </ul>
        </div>
          ...
          <div className="rounded-[42px] bg-[var(--green)] p-4 shadow-2xl shadow-[rgba(14,90,69,0.2)]">
            <div className="rounded-[32px] bg-[var(--cream)] p-7 md:p-10">
              <div className="flex items-center justify-between">
                <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-[var(--green)]">Today&apos;s lesson</span>
              ...
              <div className="mt-10 rounded-3xl bg-white p-7 shadow-sm">
                ...
                <p className="mt-5 text-sm leading-7 text-[var(--muted)]">Personalized Quran reading with teacher feedback, lesson progress and practice goals.</p>
                ...
                <div className="mt-2 flex justify-between text-xs text-slate-500"><span>Course progress</span><span>78%</span></div>
              </div>
              <div className="mt-5 grid grid-cols-2 gap-4">
                <div className="rounded-2xl bg-white p-4"><p className="text-xs text-slate-500">Teacher</p><strong className="mt-1 block">Ustadh Ahmed</strong></div>
                <div className="rounded-2xl bg-white p-4"><p className="text-xs text-slate-500">Next class</p><strong className="mt-1 block">7:00 PM</strong></div>
              </div>
            </div>
          </div>
        </div>
    ...
    <section id="programs" aria-labelledby="programs-title" className="mx-auto max-w-7xl px-6 py-24">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {programs.map(([title, category, level, description]) => (
          <CourseCard key={title} title={title} category={category} level={level} description={description} />
        ))}
      </div>
    </section>
  );
...
export function HowItWorks() {
  const steps = [["01", "Book a free trial"], ["02", "Meet your teacher"], ["03", "Choose your schedule"], ["04", "Learn & grow"]];
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_.9fr] lg:items-center">
          ...
          <form action="/api/trial" method="post" className="rounded-3xl bg-white p-6 shadow-sm">
            <div className="grid gap-4 sm:grid-cols-2">
              <input aria-label="Student name" id="student-name" name="name" required maxLength={100} autoComplete="name" className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="Student name" />
              <input aria-label="Parent name" id="parent-name" name="parentName" maxLength={100} autoComplete="name" className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="Parent name" />
              <input aria-label="Email" id="email" name="email" required type="email" maxLength={254} autoComplete="email" className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="Email" />
              <input aria-label="WhatsApp" id="whatsapp" name="whatsapp" maxLength={30} autoComplete="tel" className="rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="WhatsApp" />
              <select aria-label="Program" id="program" name="program" className="rounded-xl border border-slate-200 px-4 py-3 sm:col-span-2">
                <option>Quran Reading</option><option>Tajweed</option><option>Hifz</option><option>Islamic Studies</option>
              </select>
              <input aria-hidden="true" tabIndex={-1} autoComplete="off" name="website" className="hidden" />
            </div>
            <button type="submit" className="mt-4 w-full rounded-xl bg-[var(--green)] px-5 py-3 font-semibold text-white transition hover:bg-[var(--green-dark)]">Request Free Trial</button>
          </form>
        </div>
      </div>
    </section>
